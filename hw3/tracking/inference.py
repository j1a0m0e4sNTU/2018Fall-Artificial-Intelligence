        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for pos, prob in newPosDist.items():
                beliefsNew[pos] += prob * self.beliefs[oldPos]

        beliefsNew.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        uniform = [1 for _ in range(len(self.legalPositions))]
        self.particles = util.nSample(uniform, self.legalPositions, self.numParticles)
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        newBbeliefs = util.Counter()
        if noisyDistance == None:
            newBbeliefs[self.getJailPosition()] = 1
        else:
            for pos in self.particles:
                dist = util.manhattanDistance(pos, pacmanPosition)
                newBbeliefs[pos] += emissionModel[dist]

        if newBbeliefs.totalCount() == 0:
            self.initializeUniformly()
        else:
            newBbeliefs.normalize()
            self.particles = [util.sample(newBbeliefs) for _ in range(len(self.particles))]
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None: 
                prisonedGhostId.append(i)
        
        for particle in self.particles:
            for i in prisonedGhostId:
