...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                qStateValues = [self.computeQValueFromValues(state, action) for action in actions]
                newValues[state] = max(qStateValues)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        qValue = 0
        for nextState, prob in statesProb:
    ...
    def computeActionFromValues(self, state):
        ...
        qStateValues = [self.computeQValueFromValues(state, action) for action in actions]
        maxQ = max(qStateValues)
        bestAction = actions[qStateValues.index(maxQ)]
        return bestAction
